...
func main() {
	...
	data, err := pptx.CreateWithSlides("Action & Hyperlink Smoke Test", slides)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}

	if err := os.WriteFile("sample_action_smoke.pptx", data, 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "Write error: %v\n", err)
		os.Exit(1)
	}
	fmt.Println("Created sample_action_smoke.pptx")
}
