...
func TestPlaceholderMultiChart(t *testing.T) {
	chart1 := NewBarChart([]string{"A", "B"}, []float64{10, 20}).WithTitle("Chart 1")
	chart2 := NewBarChart([]string{"C", "D"}, []float64{30, 40}).WithTitle("Chart 2")

	slides := []SlideContent{
		NewSlide("Multi Chart").
			WithPlaceholderChart(1, chart1).
			WithPlaceholderChart(2, chart2),
	}

	data, err := CreateWithSlides("Multi Chart Demo", slides)
	if err != nil {
