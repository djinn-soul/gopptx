...
func TestCenter(t *testing.T) {
	...
	cx, cy := Inches(4), Inches(2) // 3657600 x 1828800
	x, y := Center(cx, cy)
...
func TestLayoutHelpersErrors(t *testing.T) {
	...
	_, err = Stack("invalid", Point{0, 0}, 0, Size{10, 10})
	if err == nil {
		t.Error("expected error for invalid orientation in Stack")
	}

	_, err = Distribute(OrientationHorizontal, Box{0, 0, 100, 100}, 2, 60)
	if err == nil {
