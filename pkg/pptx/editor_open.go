...
func newPresentationEditorFromParts(parts map[string][]byte) (*PresentationEditor, error) {
	if _, err := requirePart(parts, contentTypesPath); err != nil {
		return nil, err
	}
	presentationXMLBytes, err := requirePart(parts, presentationXMLPath)
	if err != nil {
		return nil, err
	}
	presentationRelsBytes, err := requirePart(parts, presentationRelPath)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("parse %s: %w", presentationRelPath, err)
	}
	slideIDRefs, err := parsePresentationSlideIDs(presentationXMLBytes)
	if err != nil {
		return nil, fmt.Errorf("parse %s: %w", presentationXMLPath, err)
	}
	...
	editor.metadata = PresentationMetadata{
		Title:      extractCoreTitle(parts[corePropsPath]),
...
func loadPackageParts(filePath string) (map[string][]byte, error) {
	...
	for _, entry := range zr.File {
		...
		parts[canonicalPartPath(entry.Name)] = data
	}
	return parts, nil
}
...
func resolveSlideReferences(
	...
	rels []editorRelationship,
	parts map[string][]byte,
) ([]editorSlideRef, []editorRelationship, error) {
	relByID := make(map[string]editorRelationship, len(rels))
	nonSlide := make([]editorRelationship, 0, len(rels))
	...
	out := make([]editorSlideRef, 0, len(slideIDs))
	for _, item := range slideIDs {
		...
		partName := canonicalPartPath(path.Join("ppt", target))
...
func parseRelationshipsXML(content []byte) ([]editorRelationship, error) {
	decoder := xml.NewDecoder(bytes.NewReader(content))
	out := make([]editorRelationship, 0, 8)
