...
func TestAnimation_Creation(t *testing.T) {
	...
	if a.ShapeIndex != 1 || a.Effect != AnimationEntranceAppear { t.Error("Basic props failed") }
	if a.DurationMS != 1500 || a.DelayMS != 500 || a.Trigger != AnimationOnClick { t.Error("Time/Trigger failed") }
	if a.RepeatCount != 2 || !a.AutoReverse { t.Error("Extra props failed") }
}
...
func TestAnimation_XML(t *testing.T) {
	...
	xml := a.XML(1, 10)
	if !strings.Contains(xml, `repeatCount="2000"`) { t.Error("Repeat XML failed") }
	if !strings.Contains(xml, `autoRev="1"`) { t.Error("AutoRev XML failed") }
	if !strings.Contains(xml, `spid="10"`) { t.Error("ShapeID XML failed") }
}
