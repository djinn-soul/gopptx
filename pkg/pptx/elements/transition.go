...
func (o TransitionOptions) XML() string {
	...
	if !o.DisableAdvanceOnClick {
		// Default is true (advance on click), but XML attr 'advClick' default is true.
		// Wait, test 'push right auto advance' expects advClick="0".
		// That means DisableAdvanceOnClick=true -> advClick="0".
		// If Disable=false (default), advClick="1" (default).
	} else {
		b.WriteString(` advClick="0"`)
	}
	if o.AdvanceAfterMS > 0 {
		b.WriteString(` advTm="`)
		b.WriteString(strconv.FormatUint(uint64(o.AdvanceAfterMS), 10))
		b.WriteString(`"`)
	}
	if o.DurationMS > 0 {
		b.WriteString(` dur="`)
		b.WriteString(strconv.FormatUint(uint64(o.DurationMS), 10))
		b.WriteString(`"`)
	}
	b.WriteString(`><p:`)
	...
	if o.Orientation != "" {
		b.WriteString(` orient="`)
		b.WriteString(string(o.Orientation))
		b.WriteString(`"`)
	}
	if o.SpokeCount > 0 {
		b.WriteString(` spokes="`)
		b.WriteString(strconv.FormatUint(uint64(o.SpokeCount), 10))
		b.WriteString(`"`)
	}
	if o.ThruBlk {
