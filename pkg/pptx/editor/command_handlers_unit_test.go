...
func TestCommandHandlers_Content(t *testing.T) {
	...
	t.Run("Shapes", func(t *testing.T) {
		...
		payload := []byte(
			`{"slide_index": 1, "type": "rect", "x": 100.0, "y": 100.0, "w": 100.0, "h": 100.0, "text": "Hello"}`,
		)
		...
		if err != nil {
			t.Fatalf("handleListShapes failed: %v", err)
		}
		if listResp == nil {
			t.Error("expected shape list")
		}

		// Update Shape
		upPayload := []byte(
			`{"slide_index": 1, "shape_id": ` + strconv.Itoa(shapeID) + `, "updates": {"text": "New Text"}}`,
		)
		_, err = handleUpdateShape(e, upPayload)
		if err != nil {
			t.Fatalf("handleUpdateShape failed: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("handleSearchShapes failed: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("handleRemoveShape failed: %v", err)
		}
		...
		imgPayload := []byte(
			`{"slide_index": 1, "path": "` + filepath.ToSlash(
				dummyImg,
		...
		imgBytesNoFormatPayload := []byte(
			`{"slide_index": 1, "data": "AQID", "x": 0.0, "y": 0.0, "w": 50.0, "h": 50.0}`,
		)
	...
	t.Run("CommentsAndNotes", func(_ *testing.T) {
	...
	t.Run("Tables", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("handleGetTable failed: %v", err)
		}

		// Update Table Cell
		updPayload := []byte(
			`{"slide_index": 1, "shape_id": ` + strconv.Itoa(
				tableID,
			) + `, "row": 0, "col": 0, "updates": {"text": "Cell 0,0"}}`,
		)
		_, err = handleUpdateTableCell(e, updPayload)
		if err != nil {
			t.Fatalf("handleUpdateTableCell failed: %v", err)
		}

		// Update Table Flags
		flagsPayload := []byte(
			`{"slide_index": 1, "shape_id": ` + strconv.Itoa(tableID) + `, "flags": {"first_row": true}}`,
		)
		_, err = handleUpdateTableFlags(e, flagsPayload)
		if err != nil {
			t.Fatalf("handleUpdateTableFlags failed: %v", err)
		}

		// Merge Cells
		mergePayload := []byte(
			`{"slide_index": 1, "shape_id": ` + strconv.Itoa(tableID) + `, "row1": 0, "col1": 0, "row2": 1, "col2": 1}`,
		)
		_, err = handleMergeTableCells(e, mergePayload)
		if err != nil {
			t.Fatalf("handleMergeTableCells failed: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("handleSplitTableCell failed: %v", err)
		}
	})
}
