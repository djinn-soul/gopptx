...
func FuzzContentTypes(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, slideCount, chartCount, smartArtCount, customXMLCount, masterCount, notesThemeIndex, embeddedFontsCount, imageExtCount, commentSlidesCount int) {
		// Limit counts to avoid OOM during fuzzing
		if slideCount < 0 || slideCount > 100 { slideCount = 10 }
		if chartCount < 0 || chartCount > 100 { chartCount = 10 }
		if smartArtCount < 0 || smartArtCount > 100 { smartArtCount = 10 }
		if customXMLCount < 0 || customXMLCount > 100 { customXMLCount = 10 }
		if masterCount < 0 || masterCount > 100 { masterCount = 10 }
		if notesThemeIndex < 0 || notesThemeIndex > 100 { notesThemeIndex = 10 }
		if embeddedFontsCount < 0 || embeddedFontsCount > 100 { embeddedFontsCount = 10 }
		if imageExtCount < 0 || imageExtCount > 10 { imageExtCount = 2 }
		if commentSlidesCount < 0 || commentSlidesCount > 100 { commentSlidesCount = 10 }

		exts := []string{"png", "jpg"}
		if imageExtCount > 0 {
			exts = make([]string, imageExtCount)
			for i := 0; i < imageExtCount; i++ {
...
func FuzzSectionListXML(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, name, guid string, slideID int64) {
		sections := []Section{
			{
				Name:     name,
				GUID:     guid,
...
func FuzzEmbeddedFontsXML(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, typeface, style string, charset uint8, panose string, pitchFamily uint8, relID string) {
...
func FuzzRichTextRun(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, text string, bold, italic bool, underline, strikethrough string, subscript, superscript bool, color, highlight, font string, sizePt int, code, allCaps, smallCaps bool) {
		run := TextRunSpec{
			Text:          text,
			Bold:          bold,
			Italic:        italic,
