...
import pathlib
import pytest
from gopptx import Presentation, GopptxError, SHAPE_RECTANGLE

project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
input_deck = os.path.join(project_root, "examples/assets/01/01_basic_pptx.pptx")

def test_presentation_content_basic():
    ...
    with Presentation(input_deck) as prs:
        ...
        shape_id = slide.add_shape(SHAPE_RECTANGLE, 100, 100, 200, 200, text="Hello", properties={"fill_color": "FF0000"})
        assert shape_id > 0
...
def test_comments_and_authors():
    ...
    with Presentation(input_deck) as prs:
        ...
        # Comments
        slide_idx = 0
        comment_index = prs.add_comment(slide_idx, author_id, "This is a comment", x=100, y=100)
