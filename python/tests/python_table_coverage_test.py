...
import pathlib
import pytest
from gopptx import Presentation, GopptxError
...
def table_shape():
    ...
    with Presentation(input_deck) as prs:
        ...
        shape_id = slide.add_table(4, 4, 1000, 1000, 5000, 2000)
        yield prs, slide, shape_id

def test_cell_properties(table_shape):
...
def test_cell_split_and_batch_error(table_shape):
    prs, slide, shape_id = table_shape
    table = slide.table(shape_id)
...
def test_cell_range_merge_batch_error(table_shape):
    prs, slide, shape_id = table_shape
    table = slide.table(shape_id)
...
def test_table_iter_and_cell_alias(table_shape):
    prs, slide, shape_id = table_shape
    table = slide.table(shape_id)
...
def test_table_style_flags(table_shape):
    prs, slide, shape_id = table_shape
    table = slide.table(shape_id)
    ...
    assert table.has_banded_rows is True
    table.has_banded_rows = False
    assert table.has_banded_rows is False

def test_table_indexing_errors(table_shape):
    prs, slide, shape_id = table_shape
    table = slide.table(shape_id)
...
def test_cell_repr(table_shape):
    prs, slide, shape_id = table_shape
    table = slide.table(shape_id)
    cell = table[0, 0]
    cell.text = "Hello"
