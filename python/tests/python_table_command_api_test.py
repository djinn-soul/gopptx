...
def test_table_command_api() -> None:  # noqa: D103
    ...
    with Presentation(input_deck) as prs:
        ...
        assert table["row_count"] == 3  # noqa: PLR2004, S101
        assert table["col_count"] == 3  # noqa: PLR2004, S101

        slide.set_table_cell_text(shape_id, 0, 0, "Header 1")
        cell = slide.get_table_cell(shape_id, 0, 0)
        assert cell["text"] == "Header 1"  # noqa: S101

        slide.set_table_flags(shape_id, {"first_row": True, "band_row": False})
        table = slide.get_table(shape_id)
        assert table["first_row"] is True  # noqa: S101
        assert table["band_row"] is False  # noqa: S101

        slide.merge_table_cells(shape_id, (1, 1, 2, 2))
        origin = slide.get_table_cell(shape_id, 1, 1)
        assert origin["is_merge_origin"] is True  # noqa: S101
        assert origin["row_span"] == 2  # noqa: PLR2004, S101
        assert origin["col_span"] == 2  # noqa: PLR2004, S101

        spanned = slide.get_table_cell(shape_id, 2, 2)
        assert spanned["is_spanned"] is True  # noqa: S101

        slide.split_table_cell(shape_id, 1, 1)
        origin = slide.get_table_cell(shape_id, 1, 1)
        spanned = slide.get_table_cell(shape_id, 2, 2)
        assert origin["row_span"] == 1  # noqa: S101
        assert origin["col_span"] == 1  # noqa: S101
        assert spanned["is_spanned"] is False  # noqa: S101
