...
from gopptx.slide.tables.table_collections import _as_int
...
def test_batch_context_getattr_error(prs):
    with BatchContext(prs) as batch:
...
def test_batch_context_exception_abort(prs):
    class MockPresentation:
        ...
        def begin_batch(self, **kwargs): pass
        def abort_batch(self): self.aborted = True
        def end_batch(self): return []
    mock_prs = MockPresentation()
    try:
        with BatchContext(mock_prs): raise ValueError("Abort")
...
def test_shape_mixin_extra(prs, tmp_path):
    ...
    # add_video
    v = tmp_path / "v.mp4"; v.write_bytes(b"fake video"); slide.add_video(str(v), (0,0,10,10))
    # add_audio
    a = tmp_path / "a.mp3"; a.write_bytes(b"fake audio"); slide.add_audio(str(a), (0,0,10,10))
    # add_image & metadata (use real PNG)
    img = tmp_path / "i.png"; img.write_bytes(PNG_DATA)
    sid = slide.add_image(str(img), (0,0,10,10))
    meta = slide.get_image_metadata(sid)
    assert meta is not None
    # group/ungroup
    b1 = slide.add_textbox(0,0,5,5); b2 = slide.add_textbox(5,5,5,5)
    gid = slide.group_shapes([b1, b2])
    slide.ungroup_shapes(gid)
    # Z-order
    slide.move_shape_to_front(b1); slide.move_shape_to_back(b1)
...
def test_table_collections_extra(prs):
    ...
    row = table.rows[0]; row.height = 100
    col = table.columns[0]; col.width = 100
    assert _as_int("x") == 0

def test_text_cache_mixin_extra(prs):
    slide = prs.add_slide("S1")
    sid = slide.add_textbox(0,0,10,10, text="T")
    slide._shape_text_state_cache = {sid: {"runs": [{"text": "T"}], "shape_id": sid}}
    slide._shape_records_cache = slide._shape_records()
    slide._shape_record_map = {sid: slide._shape_record_by_id(sid)}
    slide._apply_cached_run_text_update(sid, 0, "U")
    assert slide._cached_shape_record_text(sid) == "U"
    slide._replace_cached_runs(sid, [{"text": "R"}])
    slide._append_cached_run(sid, {"text": "A"})
    assert slide._cached_shape_record_text(sid) == "RA"
