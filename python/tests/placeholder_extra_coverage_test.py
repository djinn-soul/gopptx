...
def test_insert_text_style_aliases(presentation: Presentation):
    ...
    ph.insert_text("Styled Text", size=24, font_size=20, font_name="Arial", colour="FF0000")
...
def test_insert_chart_with_builders(presentation: Presentation):
    ...
    ph = next((p for p in slide.placeholders if p.placeholder_format.type in ("body", "obj", "chart")), None)
    if ph is None:
...
def test_placeholder_layout_master_refs(presentation: Presentation):
    ...
    l_ph = ph.layout_placeholder
    m_ph = ph.master_placeholder

def test_insert_picture_with_bounds(presentation: Presentation, tmp_path: pathlib.Path):
    slide = presentation.slides[0]
    ph = next((p for p in slide.placeholders if p.placeholder_format.type in ("body", "obj", "pic")), None)
    if ph is None:
...
def test_insert_table_with_bounds(presentation: Presentation):
    slide = presentation.slides[0]
    ph = next((p for p in slide.placeholders if p.placeholder_format.type in ("body", "obj", "tbl")), None)
    if ph is None:
